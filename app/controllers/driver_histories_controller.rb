class DriverHistoriesController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @driver_history.update(driver_history_params)
        ...
        format.json { render :show, status: :ok, location: @driver_history }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to driver_histories_url, notice: 'Driver history was successfully destroyed.' }
  ...
  private
    ...
    def driver_history_params
      params.require(:driver_history).permit(:date_of_incident, :value, :description, :incident_type)
    end
end
